...
- **请求体**（JSON）：
  ```json
  {
    "status": "可选参数，投诉状态，如'待处理'、'已受理'、'已解决'、None"
  }
  ```
- **响应**：
...
- **响应**：
  ...
  - 200 成功：`{"message": "检测目标用户X位，已向X位用户发送新书通知"}`
  - 400 请求参数错误：`{"error": "Missing book_id"}`
  - 404 书籍不存在或无符合条件的用户：
    - `{"error": "Book not found"}`
    - `{"error": "There are no eligible users"}`
  - 500 服务器错误：（如果邮件发送过程中出现异常，可能返回 500 错误）

- **用户筛选逻辑**：

  - 根据书籍的作者筛选用户：
    - 查询浏览过、收藏过或加入购物车过该书籍的用户。
  - 根据书籍的标签筛选用户：
    - 查询浏览过、收藏过或加入购物车过与该书籍标签相关的书籍的用户。
  - 合并用户列表并去重。
    - 根据用户是通过作者还是标签筛选出来的，构造不同的推荐理由。

- **触发条件**：

  - 管理员上新图书并完成标签后，可以选择触发
