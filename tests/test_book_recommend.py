...
@pytest.fixture(scope='function')
def setup_data(client, login_user):
    # 设置测试数据
    user_id = login_user
...
def test_book_recommend(client, setup_data):
    user_id = setup_data
    response = client.get('/api/recommend_books/recommend')
    assert response.status_code == 200
    recommendations = response.json['recommendations']
    assert len(recommendations) > 0

    # 检查推荐结果是否包含作者推荐和标签推荐
    author_recommendations = [rec for rec in recommendations if rec['recommend_type'] == '作者推荐']
    tag_recommendations = [rec for rec in recommendations if rec['recommend_type'] == '标签推荐']
    assert len(author_recommendations) > 0
    assert len(tag_recommendations) > 0

    # 检查推荐书籍是否符合逻辑
    for rec in author_recommendations:
        assert rec['author'] == "Author A"
    for rec in tag_recommendations:
        assert rec['recommend_reason'] in ["标签：Tag 1", "标签：Tag 2"]
